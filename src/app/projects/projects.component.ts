...
export class ProjectsComponent implements OnInit {
  ...
  pop2() {
    ...
    <br><br>
    I experimented with applying parallax effect on the clouds, creating vector graphics, using animations in the Angular style instead of the regular CSS style, and also learnt more about the quirks of CSS. This cloud app is then hosted on Microsoft Azure App Services using the free subscription (yay!) that they offer for students. I set up a <b>Continuous Integration (CI)</b> pipeline so that pushing changes to GitHub will trigger the build process on VSTS and update the Azure App automatically.
    <br><br>
    I also tried to improve accessibility and usability of the site in small ways. For instance, in the html and Angular templates, the images include an alt text to cater for visually impaired readers who use screen readers. Besides that, using contrasting colours is a recommended approach to improve the user experience of readers with colour blindness. So far, the site looks decent on mobile. In the long run, I intend to dive deeper into responsive design. 
    `;
  }
